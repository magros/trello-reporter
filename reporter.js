...
const htmlSection = (name, cards) => {
  let html = '<h4 style="margin-botom:0">' + name + '</h4>';
...
makeRequest('1/boards/' + boardId + '/lists').then(res => {
  const lists = JSON.parse(res);
  let doneList = lists.find(x => x.name.toLowerCase() === 'done');
  return doneList;
}).then(doneList => {
  ...
  sendEmail(html, 'Puntos Sprint 21 Enero - 1 Febrero - Cratos');
});
